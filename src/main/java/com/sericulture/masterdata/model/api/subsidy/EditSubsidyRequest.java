...
public class EditSubsidyRequest extends RequestBody {
    ...
    @Pattern(regexp = "^[a-zA-Z0-9\\s]*$", message = "Subsidy must contain only letters and numbers")
    @Schema(name = "subsidyName", example = "Subsidy 1 ", required = true)
    String subsidyName;

    @Pattern(regexp = "^[a-zA-Z0-9\\s\\u0C80-\\u0CFF]*$", message = "Subsidy in kannada must contain only letters and numbers")
